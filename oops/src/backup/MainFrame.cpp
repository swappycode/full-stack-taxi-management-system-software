...
void MainFrame::InitializeComponents() {
    ...
    distanceCtrl = new wxTextCtrl(bookingPanel, wxID_ANY, "");
    ...
    bookingSizer->Add(new wxStaticText(bookingPanel, wxID_ANY, "Distance (km):"), 0, wxALL, 5);
    bookingSizer->Add(distanceCtrl, 0, wxEXPAND | wxALL, 5);
...
void MainFrame::OnBookRide(wxCommandEvent& evt) {
    ...
    // Validate if selections have been made for customer, driver, and vehicle
    if (customerIndex == wxNOT_FOUND || driverIndex == wxNOT_FOUND || vehicleIndex == wxNOT_FOUND) {
        wxMessageBox("Please select a customer, driver, and vehicle", "Error", wxOK | wxICON_ERROR);
        return;
    }
    ...
    // Get actual customer name, driver name, and vehicle plate number from the selected indices
    ...
    // Create the booking using the actual data (names and plate number)
    currentBooking = new Booking(customerName, driverName, vehiclePlate);

    // Update driver and vehicle availability (mark as unavailable)
    drivers[driverIndex].setAvailability(false);
    vehicles[vehicleIndex].setAvailability(false);

    // Update the CSV files to reflect availability changes


    // Show confirmation message with the booking details
    wxMessageBox("Ride booked successfully!\n"
...
void MainFrame::OnCompleteRide(wxCommandEvent& evt) {
    ...
    std::string distanceStr = distanceCtrl->GetValue().ToStdString();
    if (distanceStr.empty()) {
        wxMessageBox("Please enter the distance", "Error", wxOK | wxICON_ERROR);
        return;
    }
    ...
    try {
        ...
        // Load the data again to ensure we have the latest state
        std::vector<Driver> drivers = Driver::loadFromCSV();
        std::vector<Vehicle> vehicles = Vehicle::loadFromCSV();

        // Complete the ride
        currentBooking->completeRide(distance);

        // Show fare
        double fare = currentBooking->getFare();
        wxMessageBox(wxString::Format("Ride completed. Your fare is: Rs. %.2f", fare), "Fare", wxOK | wxICON_INFORMATION);
        ...
        wxMessageBox("Invalid distance value", "Error", wxOK | wxICON_ERROR);
    }
}
